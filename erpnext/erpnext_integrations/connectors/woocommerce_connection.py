	...
	try:
		...
		if frappe.db.exists("Delivery Note", {"po_no": order.get('id'),"docstatus":1}):
			...
			if deliveryl.docstatus.is_submitted():
				...
				deliveryl.cancel()
				frappe.db.commit()
		if frappe.db.exists("Purchase Invoice",{"s_code": order.get('id'),"docstatus":1}):
		    purchase_invoice = frappe.get_doc("Purchase Invoice", {"s_code": order.get('id') ,"docstatus":1})
		    purchase_invoice.cancel()
		if frappe.db.exists("Purchase Receipt",{"s_code": order.get('id'),"docstatus":1}):
			purchase_receipt = frappe.get_doc("Purchase Receipt", {"s_code": order.get('id') ,"docstatus":1})
			purchase_receipt.cancel()
			frappe.db.commit()
			purchase_order = frappe.get_doc("Purchase Order", {"s_code": order.get('id') ,"docstatus":1})
...
def link_items_int(items_list, woocommerce_settings, sys_lang,orderId):
	...
	for item_data in items_list:
		...
		if(item_data.get("seller_id") == 30977):
		    item= frappe.get_doc("Item", {"sku": item_woo_com_id})
		    item_laa.append({'item_code': item.item_code , 'qty':item_data.get("quantity") ,'rate':item_data.get("cost")})
...
#	    transaction_processing([{"name": po.name}],  "Purchase Order", "Purchase Receipt")
	...
	if item_nyx is not None and item_nyx != []:
	    po = create_purchase_order(item_nyx,'31146',orderId)
#	    transaction_processing([{"name": po.name}],  "Purchase Order", "Purchase Receipt")
	#    transaction_processing([{"name": po.name}], "Purchase Order", "Purchase Invoice") Al Hakeem
...
#	    transaction_processing([{"name": po.name}],  "Purchase Order", "Purchase Receipt")
	...
	if item_de is not None and item_de != []:
	    po = create_purchase_order(item_de,'29264',orderId)
#	    transaction_processing([{"name": po.name}],  "Purchase Order", "Purchase Receipt")
	#    transaction_processing([{"name": po.name}], "Purchase Order", "Purchase Invoice") Al Hakeem
@frappe.whitelist()
	   ...
	   for order in deserialized_data :  
            try:
                ...
                if frappe.db.exists("Purchase Receipt",{"s_code": woocommerce_id,"docstatus":1}):
                    ...
                    transaction_processing([{"name": purchase_receipt.name}],  "Purchase Receipt", "Purchase Invoice")
